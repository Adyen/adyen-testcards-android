...
function fetch_release_notes_version_name() {
    ...
    release_notes_part=$(echo "$commit_message" | grep -oE 'release-notes.*')

    # Check if "release-notes" was found
    if [[ -z "$release_notes_part" ]]; then
    ...
    version_name=$(echo "$release_notes_part" | grep -oE "$version_name_regex")
